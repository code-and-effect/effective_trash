...
Automatically trash any email sent by the application, any successful user login via Devise, and all page views.

Has an effective_datatables driven admin interface to display and search/sort/filter all trash.
...
Each status, as per the config/initializers/effective_trash.rb initializer, will be created as a class level method:
...
Any log can have children trash.  This is perfect for long-running tasks where there are multiple sub events:
...
(optional) Sometimes you'd like to work with the `Effective::Trash` relationship directly.  Add to your model:

```ruby
has_many :trash, class_name: Effective::Trash, as: :associated
```
...
This behaviour can be disabled in the config/initializers/effective_trash.rb initializer.
...
```ruby
can :restore, Effective::Trash, user_id: user.id
```

and for admin:

```ruby
can :restore, Effective::Trash
...
Creating child trash via JavaScript is not yet supported.
...
```ruby
link_to 'Logs', effective_trash.admin_trash_path   # /admin/trash
```
...
If you don't want to use the builtin Admin screen, and would rather render the effective_datatable of trash elsewhere
...
We can also use a similar method to create a datatable of trash for just one user.
